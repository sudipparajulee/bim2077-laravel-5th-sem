...
@section('content')
    ...
    <form action="{{route('category.store')}}" method="POST" class="mt-5">
        ...
        <input type="text" name="name" placeholder="Enter Category Name" class="rounded w-full mt-4" value="{{old('name')}}">
        @error('name')
            <span class="text-red-600">* {{$message}}</span>
        @enderror

        <div class="flex justify-center mt-6">
            <button type="submit" class="bg-blue-600 text-white px-4 py-1 rounded mx-2">Add Category</button>
